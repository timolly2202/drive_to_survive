...
class Camera_subscriber(Node):
    def __init__(self):
        ...
        self.yolo_detections_right = YoloDetections()
        ...
        self.right_img_pub = self.create_publisher(Image, "/orange/right_image_yolo/image",1)

        self.right_detections_pub = self.create_publisher(YoloDetections, "/orange/right_image_yolo/detections",1)

    def right_camera_callback(self, input):
        image = bridge.imgmsg_to_cv2(input,"bgr8")
        output = self.model(image)

        self.yolo_detections_right.header.frame_id = "left_detections"
        self.yolo_detections_right.header.stamp = self.get_clock().now().to_msg()
        ...
        for i in output:
            ...
            for box in boxes:
                ...
                self.yolo_detections_right.bounding_boxes.append(bounding_box)

        annotated_frame = output[0].plot()
        image_msg = bridge.cv2_to_imgmsg(annotated_frame)
        self.right_img_pub.publish(image_msg)

        self.right_detections_pub.publish(self.yolo_detections_right)
        self.yolo_detections_right.bounding_boxes.clear()
