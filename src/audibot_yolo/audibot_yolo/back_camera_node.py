...
class Camera_subscriber(Node):
    def __init__(self):
        ...
        self.yolo_detections_back = YoloDetections()
        ...
        self.back_img_pub = self.create_publisher(Image, "/orange/back_image_yolo/image",1)

        self.back_detections_pub = self.create_publisher(YoloDetections, "/orange/back_image_yolo/detections",1)

    def back_camera_callback(self, input):
        image = bridge.imgmsg_to_cv2(input,"bgr8")
        output = self.model(image)

        self.yolo_detections_back.header.frame_id = "back_detections"
        self.yolo_detections_back.header.stamp = self.get_clock().now().to_msg()
        ...
        for i in output:
            ...
            for box in boxes:
                ...
                self.yolo_detections_back.bounding_boxes.append(bounding_box)

        annotated_frame = output[0].plot()
        image_msg = bridge.cv2_to_imgmsg(annotated_frame)
        self.back_img_pub.publish(image_msg)
        
        self.back_detections_pub.publish(self.yolo_detections_back)
        self.yolo_detections_back.bounding_boxes.clear()
