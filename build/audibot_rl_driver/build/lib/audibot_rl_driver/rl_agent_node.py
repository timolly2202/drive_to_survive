...
CSV_HEADER = ["episode", "timestamp", "cum_reward", "final_goal_distance_m"]
...
class AudibotRLDriverDQNNode(Node):
    def __init__(self):
        ...
        self.episode_goal_dists = []
        self._curr_ep_reward = 0.0
    ...
    def perform_rl_step_and_learn(self):
        ...
        if self.s_t_observation_buffer is not None and self.a_t_action_idx_buffer != -1:
            ...
            self._curr_ep_reward += r_t_dense_reward
    ...
    def jarred_episode_over_callback(self, msg: Float32):
        ...
        if self.latest_odom and self.goals_data:
            g = self.goals_data[self.goal_index]['centre']
            dx = g['x'] - self.latest_odom.pose.pose.position.x
            dy = g['y'] - self.latest_odom.pose.pose.position.y
            final_dist = math.hypot(dx, dy)
        else:
            final_dist = float('nan')

        self.episode_rewards.append(self._curr_ep_reward)
        self.episode_goal_dists.append(final_dist)

        csv_path = os.path.join(self.save_dir, "episode_metrics.csv")
        append_metrics(csv_path, self.current_ros_episode_num, self._curr_ep_reward, final_dist)

        self.get_logger().info(
            f"[METRICS] ep={self.current_ros_episode_num}  "
            f"reward={self._curr_ep_reward:.1f}  dist={final_dist:.2f} m"
        )
...
def append_metrics(csv_path: str, episode: int, reward: float, dist_m: float):
    """Write one row to CSV, creating the file with header if needed."""
...
def plot_metrics(csv_path: str, png_out: str | None = None):
    """Twin-axis plot <reward vs. final distance>. Saves a PNG even on headless boxes."""
    if not os.path.exists(csv_path):
        print(f"[plot_metrics] '{csv_path}' does not exist – nothing to plot.")
        return
    ...
    ax1.plot(df["episode"], df["cum_reward"].astype(float), color="tab:blue")
    ax1.tick_params(axis='y', labelcolor="tab:blue")

    ax2 = ax1.twinx()
    ax2.set_ylabel("Final dist. to goal (m)", color="tab:red")
    ax2.plot(df["episode"], df["final_goal_distance_m"].astype(float), color="tab:red")
    ax2.tick_params(axis='y', labelcolor="tab:red")

    fig.tight_layout()
    plt.title("Training progress (reward vs. goal-distance)")
...
def main(args=None):
    ...
    try:
        ...
        try:
            csv_path = os.path.join(dqn_node.save_dir, "episode_metrics.csv")
            png_path = os.path.join(dqn_node.save_dir, "training_progress.png")
            plot_metrics(csv_path, png_out=png_path)
            dqn_node.get_logger().info(f"Saved training graph → {png_path}")
        except Exception as e_plot:
            dqn_node.get_logger().error(f"Could not generate metrics plot: {e_plot}")
